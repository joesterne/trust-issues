...
The review has two layers. First, a read-only scanner does a fast pass over 15 categories of risk, from install hooks and obfuscated payloads to leaked secrets, uncommon exploit-prone bug classes, and hidden instructions. Then a five-persona adversarial read looks at what the scanner surfaced and reasons about intent, which is the part a keyword search can't do.
...
├── scripts/triage_scan.sh   read-only static triage, 15 categories
